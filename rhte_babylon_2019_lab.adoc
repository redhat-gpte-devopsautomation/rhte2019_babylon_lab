...
* Bastion server: bastion.<GUID>.internal, bastion.<GUID>.sandbox<DIGIT>.opentlc.com

* Ansible Tower nodes: tower1.<GUID>.sandbox<DIGIT>.opentlc.com, tower1.<GUID>.internal

* External primary PostgreSQL database: support1.<GUID>.sandbox<DIGIT>.opentlc.com, support1.<GUID>.internal

* Ansible Tower Isolated Nodes: worker<GUID>.{emea,na}.sandbox<DIGIT>.opentlc.com, worker{1}.{emea,na}.internal

NOTE: *<GUID>*  is 4 characters long alpha numberic string, *<DIGIT>* is 3 to 4 characters long number string which can be found in FQDN.
...
* Run playbook to create AWS ssh keypair in `eu-central-1` and `ap-southeast-2` regions. For more information refer to link:https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-key-pairs.html[ec2-key-pair] .
